...
export function Sidebar() {
    ...
    const isIconOnly = sidebar.mode === 'icon-only' || (sidebar.mode === 'icon-text' && sidebarCollapsed)

    // Calculate width
    const sidebarWidth = isIconOnly ? 72 : (sidebar.width || 260)
    ...
    return (
        <TooltipProvider delayDuration={0}>
            {/* Floating menu button when sidebar is fully collapsed */}
            {sidebarCollapsed && sidebar.mode !== 'icon-only' && (
                <Button
                    variant="outline"
                    size="icon"
            ...
            {(!sidebarCollapsed || sidebar.mode === 'icon-only') && (
                <aside
                    ...
                    className={cn(
                        ...
                        'sidebar-transition',
                        sidebar.position === 'right' && 'left-auto right-0 border-l border-r-0'
                    )}
                    role="navigation"
                ...
                >
                    ...
                    <div className={cn(
                        ...
                        {!isIconOnly && (
                            <div className="flex items-center gap-3 min-w-0">
                                {branding.logoUrl ? (
                                    ...
                                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[hsl(var(--primary))] flex-shrink-0 shadow-sm">
                                        <Sparkles className="h-5 w-5 text-[hsl(var(--primary-foreground))]" />
                                    </div>
                                )}
                                <span className="font-bold text-[hsl(var(--foreground))] truncate text-lg">
                        ...
                        {/* Toggle button - only show for icon-text mode */}
